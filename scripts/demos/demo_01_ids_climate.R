# ==============================================================================
# demo_01_ids_climate.R
# IDS Disturbance + Gridded Climate — Example Analysis
# ==============================================================================
#
# Demonstrates the core project workflow: linking IDS damage observations to
# area-weighted gridded climate summaries using the pixel decomposition pattern.
#
# Example: Mountain Pine Beetle outbreak severity vs. interannual climate.
#
#   Step 1  Load IDS disturbance data filtered to MPB (DCA_CODE 11006)
#   Step 2  Open the climate summaries lazily via Arrow
#   Step 3  Extract climate at MPB sites and compute water-year aggregates
...
#
# Prerequisites (run data compilation pipelines first):
#   01_ids/data/processed/ids_layers_cleaned.gpkg
#   processed/climate/<dataset>/damage_areas_summaries/
#
# Output: output/demo_01_ids_climate_<dataset>/  (3 PNG figures + annual_summary.csv)
#
# See also:
#   demo_02_fia_forest.R  — FIA plot/condition data and exclusion flags
#   demo_03_site_climate.R — Point-based TerraClimate for FIA sites and custom CSVs
# ==============================================================================
...
output_dir <- here("output", paste0("demo_01_ids_climate_", dataset))
...
# ==============================================================================
#
# The compiled IDS file holds 4.4 million damage observations (all agents, all
# years). A SQL query at read time filters to Mountain Pine Beetle (DCA_CODE =
# 11006), so only the ~1.3 million relevant rows are loaded into memory.
#
...
# ==============================================================================
#
# Climate summaries are stored as single parquet files — one file per variable,
# covering all 4.4M damage areas across all years.  open_dataset() connects
# without loading data into memory; queries are executed lazily.
#
# Each row: DAMAGE_AREA_ID | variable | water_year | calendar_month |
...
#
# The join is performed inside Arrow before collect(), so only MPB rows are
# pulled into memory — not the full 4.4M-area dataset.
#
...
cat(sprintf("Annual summary: %d rows written\n", nrow(annual)))

# ==============================================================================
# scripts/reshape_pixel_values.R
...
if (!dataset %in% names(dataset_configs)) {
  ...
  cat("  Available datasets: ", paste(names(dataset_configs), collapse = ", "), "\n", sep = "")
  cat("  Script will not process data.\n\n")
...
if (!processing_error) {
  ...
  output_pv_dir <- file.path(output_dir, "pixel_values")

  dir.create(output_pv_dir, recursive = TRUE, showWarnings = FALSE)
...
if (!processing_error) {
  ...
  if (length(pm_files) == 0) {
    cat("\n[ERROR] No pixel map files found\n")
    cat("  Directory checked: ", ds$pixel_map_dir, "\n", sep = "")
    cat("  Troubleshooting: Ensure pixel maps have been generated for this dataset\n\n")
    processing_error <- TRUE
  } else {
...
if (!processing_error) {
  ...
  if (length(source_files) == 0) {
    cat("\n[ERROR] No pixel value files found\n")
    cat("  Directory checked: ", ds$source_dir, "\n", sep = "")
    cat("  Expected pattern: ", ds$source_prefix, "_YYYY.parquet\n", sep = "")
    cat("  Troubleshooting: Ensure data extraction has been run for this dataset\n\n")
    processing_error <- TRUE
  } else {
    ...
    if (length(all_long) == 0) {
      ...
      cat("  Pixels checked: ", length(valid_pixels), " valid pixel IDs from pixel maps\n", sep = "")
      cat("  Files processed: ", length(source_files), " yearly files\n", sep = "")
      cat("  Troubleshooting:\n")
      cat("    - Verify pixel maps contain valid pixel_id values\n")
      cat("    - Verify source files contain matching pixel_id values\n")
      cat("    - Check that configured variables exist in source data\n\n")
      processing_error <- TRUE
    } else {
...
if (!processing_error) {
  ...
  cat("\nProcessing skipped due to errors above.\n")
}

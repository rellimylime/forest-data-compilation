...
| FIA stable-plot location input | `05_fia/data/processed/site_climate/fia_stable_plot_locations.csv` | Git-tracked | [01_build_site_list.R](../05_fia/scripts/site_climate/01_build_site_list.R) | Generated input for the optional TerraClimate extraction at all stable FIA plots |
...
| Point climate extraction | `site_climate/data/recheck/{site_pixel_map.parquet,site_climate.parquet}` | Local/scripted | [extract_terraclimate_points.R](../site_climate/scripts/extract_terraclimate_points.R) | The site-climate product that actually exists: 33,497,856 rows of monthly TerraClimate over 6,956 PI-supplied points from [site_climate/input/pi_survey_point_locations.csv](../site_climate/input/pi_survey_point_locations.csv) — 4,886 ITRDB tree-ring sites and 2,070 FIA plots. See [Point climate extraction](#point-climate-extraction) |
...
The two location inputs are intentionally named differently because they have different provenance, identifiers, and scope. The PI-supplied `pi_survey_point_locations.csv` under `site_climate/input/` is the input from which the built point-climate product was extracted. The generated `fia_stable_plot_locations.csv` under `05_fia/data/processed/site_climate/` belongs only to the optional FIA-wide extraction.

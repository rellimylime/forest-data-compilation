...
The `build_pixel_map()` function in `scripts/utils/climate_extract.R` handles both polygon and point geometries. For **point observations**, it uses `terra::cellFromXY()` to find the containing pixel and sets `coverage_fraction = 1.0`.

> **Important:** `build_pixel_map()` requires a reference raster that spans the full geographic extent of the input points. If the raster is region-limited (e.g. reconstructed from IDS pixel_values, which only covers the IDS damage area footprint), points outside that region receive garbage cell numbers from `cellFromXY()` that can coincidentally match unrelated pixel IDs — silently extracting data for wrong locations with no error. See `05_fia/cleaning_log.md` Issue #001 for a documented example of this failure.

For this reason, `05_fia/scripts/06_extract_site_climate.R` does **not** use `build_pixel_map()`. Instead it constructs a global TerraClimate raster in memory (`rast(-180, 180, -90, 90, res=1/24)`) and snaps sites directly, producing pixel IDs that are unambiguous global cell numbers matching GEE output.

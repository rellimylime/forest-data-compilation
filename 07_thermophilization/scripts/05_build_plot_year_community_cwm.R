# ==============================================================================
# 05_build_plot_year_community_cwm.R
# Build plot-visit/year community-weighted climate metrics.
#
# This script summarizes species climate niche values at the FIA plot visit level
# for each survey year. It collapses condition/subplot species records to one row
# per community layer x stable_plot_id x PLT_CN x INVYR.
#
# Supported layers:
...
#
# Output grain:
#   one row per community_layer x stable_plot_id x PLT_CN x INVYR
#
# Usage:
...
#
# Documentation:
#   07_thermophilization/README.md#script-05-inputs-and-outputs
#   07_thermophilization/README.md#plot_year_community_cwm_layerparquet
# ==============================================================================
...
out_filename <- switch(
  layer,
  seedlings = config$processed$thermophilization$files$plot_year_community_cwm_seedlings,
  saplings = config$processed$thermophilization$files$plot_year_community_cwm_saplings,
  trees = config$processed$thermophilization$files$plot_year_community_cwm_trees
)
if (is.null(out_filename) || !nzchar(out_filename)) {
  out_filename <- sprintf("plot_year_community_cwm_%s.parquet", layer)
}
out_file <- file.path(
  if (is_smoke_run) smoke_data_dir else thermo_dir,
  if (is_smoke_run) {
    sprintf("plot_year_community_cwm_%s_%s_limit_%d.parquet", layer, range_scope, limit_arg)
  } else {
    out_filename
  }
)
...
coverage_file <- file.path(qa_dir, sprintf("plot_year_community_cwm_coverage_by_state_%s.csv", qa_suffix))
missing_species_file <- file.path(qa_dir, sprintf("plot_year_community_cwm_missing_species_%s.csv", qa_suffix))
...
weighted_median_table <- function(dt, key_col, value_col, out_col) {
  ...
  sub <- dt[
    !is.na(get(value_col)) & !is.na(plot_community_weight) & plot_community_weight > 0,
    .(
      plot_visit_key = get(key_col),
      indicator_value = as.numeric(get(value_col)),
      plot_community_weight = as.numeric(plot_community_weight)
    )
  ]

  if (nrow(sub) == 0) {
    out <- data.table(plot_visit_key = character(), value = numeric())
    setnames(out, "value", out_col)
    return(out)
  }

  setorder(sub, plot_visit_key, indicator_value)
  sub[, total_weight := sum(plot_community_weight), by = plot_visit_key]
  sub[, cumulative_weight := cumsum(plot_community_weight), by = plot_visit_key]
  out <- sub[
    cumulative_weight >= total_weight / 2,
    .SD[1],
    by = plot_visit_key
  ][, .(plot_visit_key, value = indicator_value)]
  setnames(out, "value", out_col)
  out
}
...
cat("Plot-Year Community CWM Build\n")
cat("=============================\n\n")
...
if (is_smoke_run) {
  # Limit by complete plot visits so each smoke row keeps all selected conditions.
  community[, plot_visit_key := paste(stable_plot_id, PLT_CN, INVYR, sep = "|")]
  keep_visits <- unique(community$plot_visit_key)[seq_len(min(limit_arg, uniqueN(community$plot_visit_key)))]
  community <- community[plot_visit_key %in% keep_visits]
}

# ------------------------------------------------------------------------------
# Collapse to one species row per plot visit
# ------------------------------------------------------------------------------

plot_identity_cols <- intersect(
  c(
...
if (weight_col == "presence") {
  community[, base_community_weight := 1]
} else {
  community[, base_community_weight := as.numeric(get(weight_col))]
}
...
plot_condition_counts <- condition_presence[
  ,
  .(
    n_conditions_with_layer = uniqueN(CONDID),
    total_condition_prop_with_layer = sum(CONDPROP_UNADJ, na.rm = TRUE)
  ),
  by = plot_identity_cols
]

subplot_presence_cols <- c(plot_identity_cols, "CONDID", "SUBP")
subplot_presence <- unique(community[, ..subplot_presence_cols])
plot_subplot_counts <- subplot_presence[
  ,
  .(n_subplots_with_layer = .N),
  by = plot_identity_cols
]
plot_condition_counts <- merge(
...
if (length(condition_flag_cols) > 0) {
  # Preserve condition-level disturbance/treatment flags as plot-level "any"
  # summaries without splitting the plot-year CWM grain.
  plot_condition_flags <- unique(community[, c(plot_identity_cols, condition_flag_cols), with = FALSE])
  plot_condition_flags <- plot_condition_flags[
    ,
    lapply(.SD, function(x) any(as.logical(x), na.rm = TRUE)),
    by = plot_identity_cols,
    .SDcols = condition_flag_cols
  ]
} else {
  plot_condition_flags <- unique(community[, ..plot_identity_cols])
}

community_species <- community[
  ,
  .(
    plot_community_weight = sum_or_na(plot_community_weight),
...
joined[, plot_visit_key := paste(stable_plot_id, PLT_CN, INVYR, sep = "|")]

# ------------------------------------------------------------------------------
# Aggregate to plot-visit/year CWM metrics
# ------------------------------------------------------------------------------

plot_group_cols <- c("plot_visit_key", plot_identity_cols)
...
plot_metrics <- joined[
  ...
  .(
    ...
    plot_community_weight_total = sum(plot_community_weight, na.rm = TRUE),
    plot_community_weight_with_niche = sum(ifelse(has_niche, plot_community_weight, 0.0), na.rm = TRUE),
    plot_community_weight_with_study_area_niche = sum(ifelse(has_niche & niche_scope_used == "us_study_area", plot_community_weight, 0.0), na.rm = TRUE),
    plot_community_weight_with_global_fallback_niche = sum(ifelse(has_niche & niche_scope_used == "global_fallback", plot_community_weight, 0.0), na.rm = TRUE),

    cwm_temp = weighted_mean_or_na(tmean_annual_mean, plot_community_weight),
    cwm_heat = weighted_mean_or_na(tmean_warmest_month_mean, plot_community_weight),
    cwm_cold = weighted_mean_or_na(tmean_coldest_month_mean, plot_community_weight),
    cwm_temp_seasonality = weighted_mean_or_na(temp_seasonality_mean, plot_community_weight),
    cwm_cwd = weighted_mean_or_na(cwd_annual_sum, plot_community_weight),
    cwm_peak_cwd = weighted_mean_or_na(cwd_max_month_mean, plot_community_weight),
    cwm_pr = weighted_mean_or_na(pr_annual_sum, plot_community_weight),
    cwm_dry_month_pr = weighted_mean_or_na(pr_driest_month_mean, plot_community_weight),
...
median_tables <- lapply(names(indicator_map), function(short_name) {
  weighted_median_table(
    joined,
    key_col = "plot_visit_key",
    value_col = indicator_map[[short_name]],
    out_col = paste0("median_", short_name)
  )
})
for (median_dt in median_tables) {
  plot_metrics <- merge(plot_metrics, median_dt, by = "plot_visit_key", all.x = TRUE, sort = FALSE)
}

plot_metrics <- merge(plot_metrics, plot_condition_counts, by = plot_identity_cols, all.x = TRUE)
plot_metrics <- merge(plot_metrics, plot_condition_flags, by = plot_identity_cols, all.x = TRUE)
...
plot_metrics[, `:=`(
  ...
  frac_weight_with_niche = fifelse(
    ...
    plot_community_weight_with_niche / plot_community_weight_total,
    NA_real_
  ),
  ...
  frac_weight_with_study_area_niche = fifelse(
    plot_community_weight_total > 0,
    plot_community_weight_with_study_area_niche / plot_community_weight_total,
    NA_real_
  ),
  frac_weight_with_global_fallback_niche = fifelse(
    plot_community_weight_total > 0,
    plot_community_weight_with_global_fallback_niche / plot_community_weight_total,
    NA_real_
  )
)]
...
plot_metrics[, plot_visit_key := NULL]
setorder(plot_metrics, stable_plot_id, INVYR, PLT_CN)
...
summary <- data.table(
  metric = c(
    "n_plot_visit_rows",
    "n_plot_visit_rows_with_cwm_temp",
    "n_community_species_rows",
    "n_unique_species",
    "n_unique_species_missing_niche",
    "median_frac_weight_with_niche",
    "p10_frac_weight_with_niche",
    "n_plot_visit_rows_below_95pct_niche_coverage",
    "n_plot_visit_rows_using_global_fallback",
    "median_frac_weight_with_global_fallback_niche"
  ),
  value = c(
    nrow(plot_metrics),
    sum(!is.na(plot_metrics$cwm_temp)),
    nrow(community_species),
    uniqueN(community_species$SPCD),
    uniqueN(joined[has_niche == FALSE, SPCD]),
    stats::median(plot_metrics$frac_weight_with_niche, na.rm = TRUE),
    as.numeric(stats::quantile(plot_metrics$frac_weight_with_niche, 0.10, na.rm = TRUE)),
    sum(plot_metrics$frac_weight_with_niche < 0.95, na.rm = TRUE),
    sum(plot_metrics$plot_community_weight_with_global_fallback_niche > 0, na.rm = TRUE),
    stats::median(plot_metrics$frac_weight_with_global_fallback_niche, na.rm = TRUE)
  )
)
summary[, `:=`(
  community_layer = layer,
...
coverage_by_state <- plot_metrics[
  ,
  .(
    n_plot_visit_rows = .N,
    n_plot_visit_rows_with_cwm_temp = sum(!is.na(cwm_temp)),
    ...
    n_plot_visit_rows_below_95pct_niche_coverage = sum(frac_weight_with_niche < 0.95, na.rm = TRUE),
    n_plot_visit_rows_using_global_fallback = sum(plot_community_weight_with_global_fallback_niche > 0, na.rm = TRUE)
  ),
  by = state
][order(state)]
...
missing_species <- joined[
  ...
  .(
    ...
    n_plot_visit_rows = uniqueN(paste(stable_plot_id, PLT_CN, INVYR, sep = "|")),
    plot_community_weight_total = sum(plot_community_weight, na.rm = TRUE),
    n_source_rows = sum(n_source_rows, na.rm = TRUE)
  ),
  by = .(species_key, SPCD)
][order(-plot_community_weight_total, species_key)]

# ------------------------------------------------------------------------------
# Write outputs
# ------------------------------------------------------------------------------

write_parquet_safely(plot_metrics, out_file)
...
cat("\nDone.\n")
cat(glue("Plot-year CWM parquet: {out_file}"), "\n")

...
def render_newbie_concept_diagrams() -> None:
    """Prototype conceptual diagrams for Newbie mode."""
    st.subheader("How FIA Data Maps To Real Forest Measurements")
    st.caption(
        "Conceptual guide (prototype): these visuals explain the sampling design and why the database is split "
        "across PLOT, COND, TREE, SEEDLING, and reference tables."
    )
    show_table_labels = st.toggle(
        "Show FIA table names on the diagrams",
        value=False,
        key="newbie_diagram_show_table_names",
        help=(
            "Leave off for concept-first labels (plot, tree, understory). Turn on to reveal the FIADB "
            "table names that store each concept."
        ),
    )

    if show_table_labels:
        plot_label = "PLOT (+ SUBPLOT / SUBP_COND)"
        seedling_label = "SEEDLING (+ P2VEG_* for understory)"
        condition_label = "COND (+ BOUNDARY)"
        tree_label = "TREE (+ TREE_GRM_* for change)"
        ref_label = "REF_* lookup tables"
    else:
        plot_label = "Plot visit + subplot layout"
        seedling_label = "Seedlings / understory"
        condition_label = "Conditions within a plot"
        tree_label = "Trees and stems"
...
height:240px;padding:12px;">
  <div style="position:absolute;top:10px;left:14px;color:#dcecff;font-weight:700;font-size:12px;">
    Conceptual FIA forest plot scene
  </div>
  <div style="position:absolute;top:34px;left:18px;color:#b7d8ff;font-size:11px;">Canopy / overstory</div>
  <div style="position:absolute;top:78px;left:18px;color:#d4f3d5;font-size:11px;">Shrub / understory layer</div>
  <div style="position:absolute;top:128px;left:18px;color:#eef1b7;font-size:11px;">Regeneration / seedlings</div>
  <div style="position:absolute;top:174px;left:18px;color:#ebcfaa;font-size:11px;">Duff / litter / down woody material</div>

  <div style="position:absolute;left:120px;top:38px;width:10px;height:95px;background:#5b3d25;border-radius:4px;"></div>
  <div style="position:absolute;left:99px;top:20px;width:52px;height:34px;background:#2f6d41;border-radius:60% 55% 58% 52%;"></div>
  <div style="position:absolute;left:92px;top:32px;width:66px;height:30px;background:#3b844f;border-radius:50%;opacity:0.95;"></div>

  <div style="position:absolute;left:208px;top:52px;width:8px;height:82px;background:#6b472a;border-radius:4px;"></div>
  <div style="position:absolute;left:191px;top:34px;width:42px;height:28px;background:#366f45;border-radius:50%;"></div>
  <div style="position:absolute;left:188px;top:44px;width:48px;height:22px;background:#4c945c;border-radius:50%;"></div>

  <div style="position:absolute;left:280px;top:56px;width:6px;height:80px;background:#6f4a2b;border-radius:4px;"></div>
  <div style="position:absolute;left:268px;top:40px;width:30px;height:22px;background:#3f7e4d;border-radius:50%;"></div>

  <div style="position:absolute;left:142px;top:102px;width:28px;height:18px;background:#5a8a48;border-radius:50%;"></div>
  <div style="position:absolute;left:170px;top:108px;width:22px;height:16px;background:#6da55a;border-radius:50%;"></div>
  <div style="position:absolute;left:225px;top:112px;width:24px;height:16px;background:#7ebf6a;border-radius:50%;"></div>

  <div style="position:absolute;left:136px;top:132px;width:3px;height:18px;background:#9ccf7c;"></div>
  <div style="position:absolute;left:146px;top:136px;width:3px;height:14px;background:#9ccf7c;"></div>
  <div style="position:absolute;left:232px;top:136px;width:3px;height:14px;background:#9ccf7c;"></div>
  <div style="position:absolute;left:241px;top:132px;width:3px;height:18px;background:#9ccf7c;"></div>
  ...
  <div style="position:absolute;right:12px;top:14px;width:255px;border-radius:10px;padding:10px;
    ...
    with d1:
        ...
        st.markdown(
            """
<div style="position:relative;height:260px;border:1px solid #325344;border-radius:12px;
background:linear-gradient(180deg,#0f2019,#11271f);padding:12px;overflow:hidden;">
  <div style="position:absolute;top:10px;left:12px;color:#cfe8d9;font-size:12px;font-weight:600;">
    FIA plot footprint (conceptual)
  </div>
  <div style="position:absolute;top:44px;left:26px;width:76px;height:76px;border-radius:50%;
    border:2px solid #6dc497;background:#6dc49722;color:#e8fff4;font-size:11px;display:flex;
    align-items:center;justify-content:center;text-align:center;">Subplot 2<br>trees</div>
  <div style="position:absolute;top:70px;left:118px;width:82px;height:82px;border-radius:50%;
    border:2px solid #a7f2cb;background:#a7f2cb22;color:#e8fff4;font-size:11px;display:flex;
    align-items:center;justify-content:center;text-align:center;">Subplot 1<br>center</div>
  <div style="position:absolute;top:154px;left:34px;width:76px;height:76px;border-radius:50%;
    border:2px solid #6dc497;background:#6dc49722;color:#e8fff4;font-size:11px;display:flex;
    align-items:center;justify-content:center;text-align:center;">Subplot 3<br>trees</div>
  <div style="position:absolute;top:154px;left:150px;width:76px;height:76px;border-radius:50%;
    border:2px solid #6dc497;background:#6dc49722;color:#e8fff4;font-size:11px;display:flex;
    align-items:center;justify-content:center;text-align:center;">Subplot 4<br>trees</div>
  <div style="position:absolute;top:102px;left:177px;width:34px;height:34px;border-radius:50%;
    border:2px solid #ffd166;background:#ffd16622;color:#fff7da;font-size:9px;display:flex;
    align-items:center;justify-content:center;text-align:center;">microplot</div>
  <div style="position:absolute;bottom:12px;left:12px;right:12px;color:#d6e8dc;font-size:11px;line-height:1.3;">
    Trees >= 1.0 in DBH are tallied on subplots. Seedlings and some understory measures are collected on smaller microplots.
  </div>
</div>
            """,
            unsafe_allow_html=True,
        ...
        st.caption(
            "One physical visit creates records in multiple tables because crews measure different things at different nested areas."
        )

    with d2:
        st.markdown("**2. One plot can have multiple conditions**")
        st.markdown(
            """
<div style="border:1px solid #5d4d22;border-radius:12px;background:linear-gradient(180deg,#241d0f,#2b2312);
padding:12px;height:260px;">
  <div style="color:#f3e4b2;font-size:12px;font-weight:600;margin-bottom:8px;">COND explains mixed land cover within one plot</div>
  <div style="display:flex;justify-content:center;align-items:center;margin:8px 0 10px;">
    <div style="width:164px;height:164px;border-radius:50%;
      background:conic-gradient(#5abf90 0 52%, #f0b54a 52% 80%, #6ea8ff 80% 100%);
      border:2px solid #d9b35c; position:relative;">
      <div style="position:absolute;inset:48px;border-radius:50%;background:#1b1710;border:1px dashed #caa65c;
      color:#f2e4be;font-size:11px;display:flex;align-items:center;justify-content:center;text-align:center;">
        same PLOT<br>different CONDIDs
      </div>
    </div>
  </div>
  <div style="display:grid;grid-template-columns:10px auto;gap:6px 8px;color:#efe3c0;font-size:11px;">
    <div style="background:#5abf90;border-radius:2px;"></div><div>Condition 1: forest stand type A</div>
    <div style="background:#f0b54a;border-radius:2px;"></div><div>Condition 2: edge / opening / different owner</div>
    <div style="background:#6ea8ff;border-radius:2px;"></div><div>Condition 3: another cover type or stand size class</div>
  </div>
</div>
            """,
            unsafe_allow_html=True,
        ...
        if show_table_labels:
            st.caption(
                "Condition geometry and splits appear across `COND`, `SUBP_COND`, and `BOUNDARY`; `CONDPROP_UNADJ` stores the area fraction per condition."
            )
        else:
            st.caption(
                "This is why `CONDPROP_UNADJ` exists: it stores the fraction of the plot represented by each condition record."
            )

    with d3:
        st.markdown("**3. Why the data is split across tables**")
        st.markdown(
            """
<div style="border:1px solid #2b4360;border-radius:12px;background:linear-gradient(180deg,#101923,#111f2b);
padding:12px;height:260px;color:#dbe9f8;">
  <div style="font-size:12px;font-weight:600;margin-bottom:10px;color:#b9d8ff;">From field measurements to database rows</div>
  <div style="display:flex;flex-direction:column;gap:8px;font-size:11px;">
    <div style="padding:8px;border-radius:8px;background:#1f3a52;border:1px solid #4d7aa3;text-align:center;">
      <strong>PLOT</strong><br>Where + when crews visited
    </div>
    <div style="text-align:center;color:#8db9e7;">|</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
      <div style="padding:8px;border-radius:8px;background:#233b2a;border:1px solid #5fa06f;text-align:center;">
        <strong>COND</strong><br>Portions of plot
      </div>
      <div style="padding:8px;border-radius:8px;background:#3d2d1f;border:1px solid #c58f57;text-align:center;">
        <strong>TREE / SEEDLING</strong><br>Stems and counts
      </div>
    </div>
    <div style="text-align:center;color:#8db9e7;">|</div>
    <div style="padding:8px;border-radius:8px;background:#302341;border:1px solid #8f72bc;text-align:center;">
      <strong>REF_*</strong><br>Decode species, forest type, ownership, disturbance codes
    </div>
  </div>
</div>
            """,
        ...
        st.caption(
            "Tables are separated because they represent different levels of measurement, not because the data was randomly split."
        )
...
ready-to-run analysis code.
        ...
        if is_newbie_mode:
            st.markdown("---")
            render_newbie_concept_diagrams()
        st.markdown("---")

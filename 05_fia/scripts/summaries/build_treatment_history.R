...
build_treatment_history <- function(out_dir, cond_ds, cond_dir = NULL) {
  ...
  rb <- fia_should_rebuild(
    ...
    required_cols = c("PLT_CN", "INVYR", "CONDID", "TRTCD", "TRTYR_calendar",
                      "treatment_label"),
    label = "plot_treatment_history"
  )
  if (!rb$rebuild) {
    cat(glue("  Up to date ({rb$reason}, {file_size(out_treat)}) - skipping\n\n"))
  } else if (is.null(cond_ds)) {
    cat("  No cond parquets found. Run 03_extract_trees.R first.\n\n")
  } else {
    if (file_exists(out_treat)) cat(glue("  Rebuilding ({rb$reason})\n"))
    ...
    if (!has_trt) {
      ...
      write_parquet_atomic(as_tibble(treat_long), out_treat, compression = "snappy")

...
The workflow produces two key output types:
  ...
  1. PIXEL MAPS  -- link observations to raster pixels
     ...
     Note: These files are large (damage_areas_pixel_map is ~156 MB) and are
           gitignored. They are regenerated by 01_build_pixel_maps.R.
...
================================================================================
LOOKUPS (committed to git)
================================================================================
Small reference files derived from the pixel maps, committed to git for use
in the project dashboard. Unlike the full pixel maps above, these are tracked
and do not need to be regenerated after cloning the repo.

  Location: 02_terraclimate/lookups/

  damage_areas_pixel_centroids.parquet
    One row per unique TerraClimate pixel that overlaps at least one IDS
    damage area. Derived from damage_areas_pixel_map.parquet by deduplicating
    on pixel_id and counting distinct DAMAGE_AREA_IDs per pixel.
    ...
    Source: damage_areas_pixel_map.parquet (regenerate with the Python snippet
            in docs/dashboard/pages/1_IDS_Survey.py if the full pixel map changes)
    Used by: docs/dashboard/pages/1_IDS_Survey.py Coverage Map tab
             Displays as a density map: each pixel colored by n_damage_areas
             (log scale) to show where IDS disturbance activity is concentrated.
